...
when new torrents appear. Failed guard runs can be retried with backoff, and
watcher state can optionally be persisted in SQLite.
...
import os, sys, json, time, signal, urllib.parse as uparse
...
from .runtime import compute_backoff_delay, is_connection_error, warn_after_attempt
from .state_store import WatcherStateStore
from .version import VERSION
from .watcher_state import merge_torrent_state, should_process
...
WATCH_STATE_DB_PATH = os.getenv("WATCH_STATE_DB_PATH", "/config/qbit-guard-state.db").strip()


def connection_warn_after() -> int:
    return warn_after_attempt(QBIT_CONNECTION_WARN_AFTER_ATTEMPT, MAX_RETRY_ATTEMPTS)
...
def main():
    ...
    state_store = None
    if WATCH_STATE_DB_PATH:
        try:
            state_store = WatcherStateStore(WATCH_STATE_DB_PATH)
            log.info("Watcher state store initialized at %s", WATCH_STATE_DB_PATH)
        except Exception as e:
            log.warning("Failed to initialize watcher state store at %s: %s", WATCH_STATE_DB_PATH, e)
    ...
    log.info(
        "Watcher started - version %s, host=%s, poll=%.1fs, process_existing_at_start=%s, rescan-keyword='%s', guard_run_max_retries=%d, max_concurrent_guards=%d",
    ...
    if state_store:
        log.info(
            "Loaded persistent watcher state: torrents=%d retry_entries=%d",
            len(torrent_state),
            len(retry_state),
        )
        ...
        while not stop["flag"]:
            try:
                ...
                for h in completed_hashes:
                    ...
                    try:
                        ...
                        if h in retry_state:
                            ...
                            if state_store:
                                state_store.delete_retry_state(h)
                        seen.add(h)
                    except Exception as e:
                        ...
                        if "404" in error_str or "Not Found" in error_str:
                            ...
                            if state_store:
                                state_store.delete_retry_state(h)
                                state_store.delete_torrent_state(h)
                            seen.add(h)
                            ...
                            if state_store:
                                state_store.upsert_retry_state(h, retry_state[h])
                            log.warning(
                ...
                for h, t in torrents.items():
                    ...
                    if state_store:
                        state_store.upsert_torrent_state(h, torrent_state[h])

                if first_snapshot:
                    first_snapshot = False
                    present = set(torrents.keys())
                    if state_store:
                        state_store.prune_missing_hashes(present)
                        retry_state = {h: state for h, state in retry_state.items() if h in present}
                        torrent_state = {h: state for h, state in torrent_state.items() if h in present}
                ...
                for h, t in torrent_state.items():
                    ...
                    ok, reason = should_process(h, t, seen, retry_state, inflight_hashes, now_ts, RESCAN_KEYWORD)
                    if not ok:
